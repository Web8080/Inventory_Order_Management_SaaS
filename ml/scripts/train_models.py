...
Implements state-of-the-art training techniques for inventory management
"""
...
class AdvancedMLTrainer:
    ...
    Implements ensemble methods, hyperparameter tuning, and model validation
    """
    ...
    def train_prophet_models(self, df: pd.DataFrame) -> Dict[str, Any]:
        ...
        for product_id, product_data in product_groups:
            if len(product_data) < 100:  # Skip products with insufficient data
                continue
            
            print(f"  🔄 Training Prophet for product {product_id}...")
            
            # Prepare data for Prophet
            prophet_df = product_data[['date', 'quantity_sold']].copy()
            ...
            try:
                ...
                print(f"    ✅ Product {product_id}: MAE={mae:.2f}, RMSE={rmse:.2f}, R²={r2:.3f}")
                
            except Exception as e:
                print(f"    ❌ Failed to train Prophet for product {product_id}: {e}")
        
        print(f"  ✅ Trained Prophet models for {len(prophet_results)} products")
    ...
    def hyperparameter_tuning(self, X_train: np.ndarray, y_train: np.ndarray) -> Dict[str, Any]:
        ...
        xgb_grid = GridSearchCV(
            xgb_model, xgb_params, cv=3, scoring='neg_mean_absolute_error', n_jobs=-1
        )
        xgb_grid.fit(X_train, y_train)
        
        print(f"  ✅ XGBoost best params: {xgb_grid.best_params_}")
        print(f"  ✅ XGBoost best score: {-xgb_grid.best_score_:.2f}")
        ...
        lgb_grid = GridSearchCV(
            lgb_model, lgb_params, cv=3, scoring='neg_mean_absolute_error', n_jobs=-1
        )
        lgb_grid.fit(X_train, y_train)
        
        print(f"  ✅ LightGBM best params: {lgb_grid.best_params_}")
        print(f"  ✅ LightGBM best score: {-lgb_grid.best_score_:.2f}")
        
        return {
            'xgboost': xgb_grid.best_estimator_,
            'lightgbm': lgb_grid.best_estimator_,
            'xgboost_params': xgb_grid.best_params_,
            'lightgbm_params': lgb_grid.best_params_
        }
    
    def save_models(self, forecasting_results: Dict, optimization_results: Dict, 
                   prophet_results: Dict, output_dir: str = "ml/models") -> Dict[str, str]:
        ...
        # Save Prophet models
        prophet_path = os.path.join(output_dir, 'prophet_models.pkl')
        joblib.dump(prophet_results, prophet_path)
        saved_models['prophet'] = prophet_path
    ...
    def generate_training_report(self, forecasting_results: Dict, optimization_results: Dict, 
                               prophet_results: Dict) -> str:
        ...
        report = {
            ...
            'prophet_models': {
                'total_models': len(prophet_results),
                'avg_mae': np.mean([r['mae'] for r in prophet_results.values()]),
                'avg_rmse': np.mean([r['rmse'] for r in prophet_results.values()]),
                'avg_r2': np.mean([r['r2'] for r in prophet_results.values()])
            }
        }
...
def main():
    ...
    prophet_results = trainer.train_prophet_models(features_df)
    
    # Hyperparameter tuning
    print("\n🔧 Hyperparameter Tuning...")
    X_train, X_test, y_train, y_test = train_test_split(
        features_df[forecasting_results['feature_columns']], 
        features_df['target_quantity'].dropna(), 
        test_size=0.2, 
        random_state=42
    )
    ...
    # Save models
    print("\n💾 Saving Models...")
    saved_models = trainer.save_models(forecasting_results, optimization_results, prophet_results)
    
    # Generate report
    print("\n📊 Generating Training Report...")
    report_path = trainer.generate_training_report(forecasting_results, optimization_results, prophet_results)
    ...
    print(f"\n📈 Prophet Models:")
    print(f"  • Models Trained: {len(prophet_results)}")
    print(f"  • Average MAE: {np.mean([r['mae'] for r in prophet_results.values()]):.2f}")
    print(f"  • Average R²: {np.mean([r['r2'] for r in prophet_results.values()]):.3f}")
    ...
    print(f"\n🚀 Next Steps:")
    print(f"  1. Start ML service: uvicorn ml.main:app --reload --port 8001")
    print(f"  2. Test predictions: python ml/scripts/test_predictions.py")
    print(f"  3. Deploy models: python ml/scripts/deploy_models.py")
